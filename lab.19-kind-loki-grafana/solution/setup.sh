...
lsof -ti:3100 | xargs kill -9 || echo "No process found on port 3100"

# Run kind cluster
CLUSTER_NAME=$(basename "$(dirname "$(pwd)")")
kind delete cluster --name $CLUSTER_NAME --config kind-config.yaml
...
kubectl port-forward -n grafana service/grafana 3000:3000 2>&1 > /dev/null &
kubectl port-forward -n alloy service/alloy 12345:12345 2>&1 > /dev/null &
kubectl port-forward -n loki service/loki-gateway 3100:3100 2>&1 > /dev/null &
